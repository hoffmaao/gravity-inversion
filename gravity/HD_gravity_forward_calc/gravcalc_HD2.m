function gravcalc_HD2(LN,A,rhoi,rhosed,rhoback,dm)
...
%        P17 or P18.
%   A = a matrix of PolarGAP data with a column of sediment depth added
%   rhoi = ice density (kg m^-3)
%   rhosed = sediment density (kg m^-3)
%   rhoback = background rock density (kg m^-3)
%   dm = a value of the gravity-modeling intervals in meters, 2000 m is
%        enough and the calculation is fast
% 
% Output
%   A figure of the ice-surface and bed topography, sediments and gravity disturbances
...
% rhoi = 900;             % ice
% rhoback = 2670;         % background (bedrock, assumes standard crystalline rock)

% Convert to density contrast, which is what the forward gravity
% calculation takes.
rho = [rhoi+rhoback;rhosed+rhoback;rhoback*2;rhoi;rhosed]; 

%% Ice-surface and bed topography

m0 = [xobs -A(:,13) -A(:,11) -A(:,14)];
id = isnan(m0(:,2));
m0(id,:) = [];
...
load boug_goce5000
% load boug_goce5000_BM
% boug_goce5000 = boug_goce5000_BM;
